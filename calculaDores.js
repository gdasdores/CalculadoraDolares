...
async function validate() {
    ...
    if (inputMoneda == 0) {
        ...
        if (dondeCompra2 == "En el banco (Dólar ahorro)"){
          ...
          outputText = "Tu compra por un total de <strong>$ " + inputMoneda.replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " dólares </strong> va a costarte: <strong><br/><br/><font size='+3'>$" + dolarLiquiCalc.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "</strong></font><br/><br/> Cotizaciones al <strong>" + date + "</strong> del dólar CCL: </br><strong>$ " + dolarLiquiFin + " </strong>";
        }
        else if (dondeCompra2 == "Compra de bonos en pesos y venta en dólares (MEP - Dólar Bolsa)"){
          outputText = "Tu compra por un total de <strong>$ " + inputMoneda.replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " dólares </strong> va a costarte: <strong><br/><br/><font size='+3'>$" + dolarBolsaCalc.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "</strong></font><br/><br/> Cotizaciones al <strong>" + date + "</strong> del dólar MEP: </br><strong>$ " + dolarBolsaFin + " </strong>";
        }
